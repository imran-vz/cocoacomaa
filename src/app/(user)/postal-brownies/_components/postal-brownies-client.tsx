...
const brownieComboSchema = z.object({
	selectedCombo: z
		...
		.min(1, "Please select a brownie combo."),
});
...
export default function PostalBrowniesClient({
		...
		onSubmit: async ({ value }) => {
			...
			try {
				const combo = postalCombos.find(
					(c) => c.id.toString() === value.selectedCombo,
				);
				if (!combo) return;
	...
	const selectedComboId = useStore(
		form.store,
		(state) => state.values.selectedCombo,
	);
							...
							children={(field) => {
								...
								return (
									<Field data-invalid={hasErrors}>
										<FieldLabel className="text-lg sm:text-xl font-semibold block text-center">
