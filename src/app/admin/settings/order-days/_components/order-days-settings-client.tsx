...
interface OrderDaysSettingsClientProps {
	initialSettings: CakeOrderSettings;
}

export function OrderDaysSettingsClient({
	initialSettings,
}: OrderDaysSettingsClientProps) {
	const queryClient = useQueryClient();
	const [selectedDays, setSelectedDays] = useState<number[]>(
		initialSettings.allowedDays,
	);
	...
	const updateSettingsMutation = useMutation({
		mutationFn: async ({
			...
			const response = await fetch("/api/cake-order-settings", {
				...
				body: JSON.stringify({ allowedDays, isActive, id: initialSettings.id }),
			});
